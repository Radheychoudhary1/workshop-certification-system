	...
	return (
		<div className="container-fluid bg-light min-vh-100 py-4 px-3 px-md-5">
			...
			<div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center bg-dark rounded-3 px-4 py-3 shadow mb-4 gap-3">
				...
				<button className="btn btn-warning text-dark fw-semibold" onClick={() => navigate("/dashboard")}>
					← Back to Dashboard
				</button>
			</div>
			...
			<div className="bg-white rounded-4 shadow-sm p-4 border border-light-subtle">
				...
				<div className="mb-3">
					...
					<input type="text" className="form-control" value={workshopName} onChange={(e) => setWorkshopName(e.target.value)} required />
				</div>
				...
				<div className="mb-3">
					...
					<input id="datetimeInput" type="datetime-local" className="form-control" style={{ cursor: "pointer" }} value={dateTime} onChange={(e) => setDateTime(e.target.value)} required />
				</div>

				<div className="mb-4">
					<label className="form-label fw-semibold">Instructions</label>
					<textarea className="form-control" rows={4} value={instructions} onChange={(e) => setInstructions(e.target.value)} placeholder="Enter workshop instructions" />
				</div>

				{/* Action Buttons */}
				<div className="d-flex gap-3 mb-4">
					<button className="btn btn-warning fw-semibold" onClick={handleSaveDraft} disabled={loading || !isFormValid}>
						Save Draft
					</button>

					<button className={`btn fw-semibold ${ active ? "btn-danger" : "btn-success" }`} onClick={handleToggleActive} disabled={loading || !formId}>
				...
				{active && formId && (
					<div>
						...
						<div className="input-group">
							<input type="text" className="form-control" readOnly value={`${window.location.origin}/form/${formId}`} />
							<button className="btn btn-outline-warning" onClick={() => handleCopy(`${window.location.origin}/form/${formId}`) }>
